...
class BlowfishCipher(Cipher):
    ...
    def _f_function(self, arg: int) -> int:
        ...
        temp = temp ^ self._S_BOX[2][arg >> 8 & 0xff]
        temp = (temp + self._S_BOX[3][arg & 0xff]) % 2 ** 32
        return temp
    ...
    def _init_blowfish(self):
        ...
        self._generate_sub_keys(self._key)
        left = 0x00000000
        right = 0x00000000

        # In order to continue the blowfish key schedule, a 64-bit zero block is split into left and right and it is
        # Encrypted, replacing P1 and P2, the resulting P1 and P2 are encrypted again, and they replace P3 and P4. This
        # Continues until the P-BOX has been rewritten.
        for i in range(0, 17, 2):
            left, right = self._encrypt_block(left, right)
            self._P_BOX[i] = left
            self._P_BOX[i + 1] = right

        # The chain of encrypting the 64 bit zero block continues replacing all the substitution boxes as well in the
        # Same manner as the P-BOX.
        for i in range(4):
            for j in range(0, 255, 2):
                left, right = self._encrypt_block(left, right)
                self._S_BOX[i][j] = left
                self._S_BOX[i][j + 1] = right
    ...
    def _generate_sub_keys(self, key: str) -> None:
        ...
        j = 0
        for idx, p_itm in enumerate(self._P_BOX):
            self._P_BOX[idx] = p_itm ^ int(key[j:j + 8], 16)
            j = (j + 8) % len(key)
    ...
    def encrypt(self, plain_text: bytes) -> bytes:
        ...
        # Splits the padded plain text int blocks of 8 bytes each
        blocks = [padded[i:i + 8] for i in range(0, len(padded), 8)]
        encrypted_blocks = list()

        for block in blocks:
            block = int.from_bytes(block, 'big')
            # Splits the 8 byte block into the first 4 bytes (left) and last 4 bytes(right)
            left = block >> 32
            right = block & 0xffffffff
            e_left, e_right = self._encrypt_block(left, right)
            # After the left and right are encrypted, join the bytes together again and convert back to bytes
            e_block = (e_left << 32) ^ e_right
            encrypted_blocks.append(e_block.to_bytes(8, 'big'))
    ...
    def decrypt(self, ciphertext: bytes) -> bytes:
        ...
        # Splits the cipher text into blocks of 8 bytes
        blocks = [ciphertext[i:i + 8] for i in range(0, len(ciphertext), 8)]
        for block in blocks:
            block = int.from_bytes(block, 'big')
            # Same process as the encryption but decrypting.
            left = block >> 32
            right = block & 0xffffffff

            d_left, d_right = self._decrypt_block(left, right)
            d_block = (d_left << 32) ^ d_right
